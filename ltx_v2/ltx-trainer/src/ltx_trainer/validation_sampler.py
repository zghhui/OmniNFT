...
class GenerationConfig:
    ...
    guidance_scale: float = 4.0  # CFG guidance scale (unified fallback)
    video_guidance_scale: float | None = None  # Per-modality CFG for video (None → use guidance_scale)
    audio_guidance_scale: float | None = None  # Per-modality CFG for audio (None → use guidance_scale)
...
class ValidationSampler:
    ...
    def _run_denoising(
        ...
        _video_cfg = config.video_guidance_scale if config.video_guidance_scale is not None else config.guidance_scale
        _audio_cfg = config.audio_guidance_scale if config.audio_guidance_scale is not None else config.guidance_scale
        video_cfg_guider = CFGGuider(_video_cfg)
        audio_cfg_guider = CFGGuider(_audio_cfg)
        ...
        with torch.autocast(device_type=str(device).split(":")[0], dtype=torch.bfloat16):
            for step_idx, sigma in enumerate(sigmas[:-1]):
                if step_callback is not None:
                    step_callback(step_idx, total_steps)
                ...
                # Apply CFG if guidance_scale != 1.0 (per-modality)
                if (video_cfg_guider.enabled() or audio_cfg_guider.enabled()) and v_ctx_neg is not None:
                    ...
                    if video_cfg_guider.enabled():
                        denoised_video = denoised_video + video_cfg_guider.delta(pos_video, neg_video)
                    if audio is not None and denoised_audio is not None and audio_cfg_guider.enabled():
                        denoised_audio = denoised_audio + audio_cfg_guider.delta(pos_audio, neg_audio)
    ...
    def _encode_prompts(
        ...
        _v_cfg = config.video_guidance_scale if config.video_guidance_scale is not None else config.guidance_scale
        _a_cfg = config.audio_guidance_scale if config.audio_guidance_scale is not None else config.guidance_scale
        if _v_cfg != 1.0 or _a_cfg != 1.0:
